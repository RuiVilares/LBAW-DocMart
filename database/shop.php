<?php
  ...
  function getArticles($num) {
    ...
    $stmt = $conn->prepare('SELECT id, preco, caminho_da_fotografia, nome, descricao FROM "Artigo"
      WHERE 0 < (SELECT sum(quantidade) FROM "Item" WHERE "Artigo".id = "Item".id_artigo) ORDER BY id DESC LIMIT :nItems;');

    $stmt->bindParam(':nItems', $num, PDO::PARAM_INT);
  ...
  function getConditions($colors, $sizes, $minPrice, $maxPrice) {
    ...
    if (!is_null($colors)) {
      ...
      $conditions .= "(cor IN (";
      for ($i = 0; $i < $last; $i++) {
          $conditions = $conditions . ":color" . $i .", ";
      }

      $conditions .= ":color" . $last . ")) ";
    }
    ...
    if ($sizes != null) {
      ...
      $conditions .= "(tamanho IN (";
      for ($i = 0; $i < $last; $i++) {
          $conditions = $conditions . " :size" . $i .", ";
      }

      $conditions .= " :size" . $last . ")) ";
    }
  ...
  function getItems($genre, $type, $nItems, $page, $lastId, $colors, $sizes, $minPrice, $maxPrice) {
    ...
    if ($lastId >= 0) {
      $stmt = $conn->prepare('SELECT DISTINCT ON ("Artigo".id) "Artigo".id, preco, caminho_da_fotografia, nome, descricao FROM "Artigo" INNER JOIN "Item" ON "Artigo".id = "Item".id_artigo
        WHERE genero = :genre AND tipo = :type AND "Artigo".id >= :id AND quantidade > 0 ' . $conditions . ' 
        ORDER BY id LIMIT :nItems;');
    } else {
      $stmt = $conn->prepare('SELECT DISTINCT ON ("Artigo".id) "Artigo".id, preco, caminho_da_fotografia, nome, descricao FROM "Artigo" INNER JOIN "Item" ON "Artigo".id = "Item".id_artigo
        WHERE genero = :genre AND tipo = :type AND "Artigo".id < :id AND quantidade > 0' . $conditions . '
    ...
    if ($colors != null) {
      ...
      foreach ($colors as $valColors) {
        $stmt->bindParam(':color' .$i, $colors[$i], PDO::PARAM_STR);
        $i++;
      }
    }
    if ($sizes != null) {
      ...
      foreach($sizes as $valSizes) {
        $stmt->bindParam(':size' .$i, $sizes[$i], PDO::PARAM_STR);
        $i++;
      }
    }
    if($minPrice != null){
  ...
  function countValidItems($genre, $type, $colors, $sizes, $minPrice, $maxPrice) {
    ...
    $stmt = $conn->prepare('SELECT count(*) FROM "Artigo" INNER JOIN "Item" ON "Artigo".id = "Item".id_artigo
      WHERE genero = :genre AND tipo = :type AND quantidade > 0 ' . $conditions . ' GROUP BY "Artigo".id;');
